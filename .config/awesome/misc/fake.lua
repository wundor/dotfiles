...
local function create_fake(s)
    ...
    s.fake = _G.screen.fake_add(geo.x + real_w, geo.y, fake_w, geo.height)
    s.fake.parent = s
    -- Mark screens
    s.fake.is_fake = true
    s.has_fake = true
...
local function remove_fake(s)
    ...
    if not s.has_fake then
        return
    end
    -- Index for cleaner code
    local index = tostring(s.index)
    s:fake_resize(screens[index].geo.x, screens[index].geo.y, screens[index].geo.width, screens[index].geo.height)
...
local function toggle_fake(s)
    ...
    if not s.has_fake and not s.is_fake then
        return
    end
    -- Ge real screen if fake was focused
    if s.is_fake then
    ...
    if s.fake.is_open then
        ...
        s.fake:fake_resize(screens[index].geo.width, screens[index].geo.y, 1, screens[index].geo.height)
        -- Mark fake as hidden
        s.fake.is_open = false
    else -- Fake was selected
        -- Resize screens
        s:fake_resize(screens[index].geo.x, screens[index].geo.y, screens[index].real_w, screens[index].geo.height)
        s.fake:fake_resize(screens[index].geo.x + screens[index].real_w, screens[index].geo.y, screens[index].fake_w,
...
local function reset_fake(s)
    ...
    if s.fake.is_open then
        ...
        s:fake_resize(screens[index].geo.x, screens[index].geo.y, screens[index].real_w, screens[index].geo.height)
        s.fake:fake_resize(screens[index].real_w, screens[index].geo.y,
            screens[index].geo.width - screens[index].real_w, screens[index].geo.height)
    end
    -- Because memory leak
    collectgarbage('collect')
    -- Emit signal
...
_G.screen.connect_signal('remove_fake', function(s)
    remove_fake(s)
end)
_G.screen.connect_signal('toggle_fake', function(s)
    toggle_fake(s)
end)
_G.screen.connect_signal('create_fake', function(s)
    create_fake(s)
end)
_G.screen.connect_signal('resize_fake', function(a, s)
    resize_fake(a, s)
end)
_G.screen.connect_signal('reset_fake', function(s)
    reset_fake(s)
end)
